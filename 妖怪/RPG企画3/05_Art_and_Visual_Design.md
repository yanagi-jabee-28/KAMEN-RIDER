...
* **黄泉戸喫UIの美術:** 黄泉戸喫状態を示すHPゲージは、和紙に黒墨が染み込み、金継ぎの輝きが走る独特のグラフィックとする。回復時の痛み演出としてゲージが赤黒いノイズを放つアニメーションも規定する。また、黄泉戸喫を行ったキャラクターの立ち絵やモデルには、肌に浮かぶ黒い墨の侵食痕や衣服から覗く痛々しい金継ぎのラインが永続的に追加される。
  - **ホラー的厚塗りの回避:** 「泥臭さ」は、グロテスクな表現ではなく、あくまで生命の温かみや土の匂いが感じられる範囲に留める。キャラクターの造形美は常に担保される。
  - **武器デザイン（情念と摩擦の出力装置）:** 武器は単なるダメージリソースではなく「天の理（神の完璧な予測）を狂わせるためのノイズ出力装置」として描く。西洋的な洗練された金属光沢やファンタジー意匠は排除し、使用素材（石・骨・青銅・鉄滓・木・麻・和紙・泥）の野趣あふれる質感を強調する。和紙と岩絵具の筆致を用い、王道武器（剣、弓など）は機能美の中に錆や修復跡（極太の金継ぎ）を痛々しく映し出す。日用品／祭具カテゴリ（御幣、双鎌、神楽面、巨大やっとこ、漁網、和傘、釣竿など）は、それを振り回す行為自体が神への痛烈な反逆（ノイズ）であることを示すため、更に奔放な滲みや激しい掠れ、不規則な岩絵具の飛沫をまとわせる。共有カテゴリを装備する場合でも、各キャラのトラウマや戦闘哲学（例えばマヒトの「加工硬化」やタチバナの「ピアサポート」）を反映した独自の色調と装飾で差別化する。

### 1.1 三柱の神の視覚スタイル

  * 三貴子（アマテラス・ツクヨミ・スサノオ）は同一理を共有する一群として扱い、原初の反逆者である星神カガセオは完全に独立した対極として位置付ける。三貴子側は「太陽」「月」「嵐（泥）」の天体モチーフを共有しつつ、カガセオは「星」の破片と熱暴走で描写され、人間側と同じく（あるいはそれ以上に）荒々しい滲みと熱量を用いて表現する。

- **アマテラス（凍結する光の繭）:**
  * シルエットは日食を想起させる垂直性のある優美な輪郭。何重にも折り重なった胡粉の衣は、重い球ではなく白百合の花弁のように展開する。
  * 滲みは極小、線は最小。和紙繊維と顔料粒子は微かに残し、外周に金環日食を思わせるコロナ状ハローを描く。
  * 表情は怯えたまなざしか瞳を閉じたまま。光そのものが引きこもりつつ、神格にふさわしい気品と華を保持する。

- **ツクヨミ（無慈悲な刃としての月）:**
  * 直線と鋭角で構成された王者の立ち姿。服の皺も折り紙のように几帳面で、長い裾と肩回りでボス級の面積支配を作る。
  * 色は冷たい胡粉白・群青・銀灰。深い夜色の和紙の上に、わずかな陰影だけを残した平面的処理で「ノイズを許さない」異質感を演出。
  * 顔は完璧な左右対称で、冷たい三白眼。所持する武器は大型の三日月弧処刑刃を連想させ、背後には満月ハローを置く。

- **スサノオ（泥に塗れた原初の嵐）:**
  * 輪郭は神として端正で力強いが、肩や衣装の縁から泥や血の飛沫が激しく飛び散るハイブリッドな表現。
  * 所持する天叢雲剣はその初期素体であり、無数の歴代ユーザーが直してきた傷を金継ぎで繋いだ痛々しい「魂の器」である。中身の魂が「真・天叢雲剣」として別の武器（神の理を打ち砕く新たな象徴）に継承された後は、彼の手元には抜け殻であるただの鉄屑が残るが、彼はそれを満足げに見届ける。
  * 色彩は嵐を連想させる鉛灰色と土色を基調にし、筆致は一部規律だが多くは奔放な滲みが混在する。
  * 根堅洲国での最終試練では、彼の動きそのものが「タイムラインの圧縮（極限の速度と密度）」を体現するように、残像とノイズが激しくブレて描かれる。

- **カガセオ（黄金のバグ／再結晶）:**
  * 元は整った神の姿が破片化し、無理やり金継ぎで繋ぎ止められた「人間的な悲劇の英雄」。アシンメトリーだが骨格と表情は読める。
  * 境界線からは盛り上がる金の漆が溢れ、星図（星座）の軌跡を思わせる結線で皮膚と装束を走る。暗土色や焦げたオレンジの熱が漏れる。
  * 滲みと細線、静謐と泥臭さが暴力的に混在。常にきしみながら再構築する不安定さの中に、痛みと気高さを同時に宿す。

- **澱神・八岐の産土（真エンドボス／幾千の悲劇のキメラ）:**
  * 単なる神話の八頭蛇ではなく、歴代の人間が「代受苦」で使い捨て、残骸となった無数の武器（遺骨）と、かつて討たれた数多の神々の絶望が寄り代となって融合した巨大な真エンドボス。
  * **前半（白化殻モード）:** 自らの傷を恐れ、反射を拒絶する真っ白な「胡粉の殻」に覆われている。無機質な滑らかさを持ち、プレイヤーの攻撃をすべて無に帰すような冷たさを表現する。
  * **後半（UIジャック・狂宴モード）:** 殻が破れると、その中から赤錆びた刃、折れた柄、極彩色の和紙の滲み（ノイズ）、そして黒く濁った黄泉の泥が暴力的に吹き出す。それら無数の破片を、太く不規則な「黄金の金継ぎ」と「黒墨」が辛うじて繋ぎ止めている。武器の墓場が動き出したようなグロテスクさと、傷跡が放つ圧倒的な熱量（美しさ）を併せ持つ。

- **アメノイワトワケノカミ（絶対孤立システムの白壁）:**
  * 天岩戸そのものであり、アマテラスの無菌室を守る絶対の装甲システム。高密度の胡粉で極端に分厚く塗り込められ、光も音も反射しない純白のフラットな巨大防壁。和紙繊維のわずかな凹凸のみが、画面奥への果てしない厚みを示唆する。キャラクター的な造形を持たない。

- **八雷神（ヤクサノイカヅチ / 黄泉の障壁）:**
  * 黄泉比良坂で「行者返し」の儀式を阻む障壁ボス。雷を纏った八体の恐ろしい異形だが、完全な獣ではなく、腐敗した甲冑と泥、そして黄泉の赤黒い熱が混在した姿。攻撃ごとに、焦げたような激しいドライブラシ（Kasure）と、紫・黒・赤濁色の暴力的滲みを見せる。

これらのビジュアルは 05_Art_and_Visual_Design.md の既存哲学と整合し、各種プロンプトを `Visual_Prompt_Pack_LUMINOUS_WASHI_V3.md` に収録して運用する。

### 1.2 三種の神器の視覚スタイルと形状制約

神話のアイテムである三種の神器は、「神の無傷な完品（イワナガ）」ではなく、「傷を負った人間に見出され、あるいは新造された不完全な器（サクヤ）」として美術デザインされる。AI生成で安易に西洋ファンタジーや近代意匠に寄ることを防ぐため、以下の明示的な物理形状制約を設ける。

- **天叢雲剣（初期素体：古代の鉄直刀）**
  - 西洋の幅広剣や十字鍔（クロスガード）、反りのある日本刀（大太刀・打刀）の形状は絶対に採用しない。
  - **極端に長くて細い日本の古代直刀（Chokuto / Tsurugi）**であり、刃幅は全長の1/10以下の平行刃となる。
  - 柄尻には**幅広の円盤状の柄頭（DISC POMMEL）**を持つ。鍔（つば）はなく、刃と柄の境目に小さな鉄のリング（鎺）があるのみ。
  - 真っ黒な艶消しの鉄の刀身は3つのセクションに分かれ、それらを繋ぐ**太く盛り上がった輝く黄金の「金継ぎ」**だけが発光し熱量を放っている。この状態は他の折れた武器の「魂」を蓄積する器として機能する。
- **真・天叢雲剣（魂を宿した覚醒状態）**
  - オロチ討伐後に覚醒する「真・天叢雲の魂」は固定の剣の形ではなく、「任意の武器に宿った魂（状態）」として視覚化される。
  - 宿主となるベース武器（刀、槍、斧、日用品など）のシルエットを保ちながら、その表面を突き破るように、極太の黄金の金継ぎと圧倒的な熱量（光）が脈打つように発光し、過熱時には画面全体に金漆のテクスチャを激しく散らす。
  - これまで呼び継ぎや代受苦で消費した歴代の武器（遺骨）の破片が、オーラや幻影のようにベース武器の周囲に明滅・付随する痛々しくも神々しいキメラ的エフェクトを持つ。
- **八咫鏡（Yata no Kagami）**
  - 発掘された朽ちた遺物やガラス鏡ではなく、マヒトによって新造された**無傷で亀裂のない「青銅の円盤鏡」**。
  - 縁は温かみのあるアンバーゴールドで縄文風の巴紋が刻まれ、中心部はオリーブグレー。
  - 完全な反射はせず、手磨きによる曇りや不均一な光の乱反射（ ghost-like internal glow ）を持つ。神の冷たい完璧さではなく、人の手作業の痕跡を残す。
- **八尺瓊勾玉（Yasakani no Magatama）**
  - コイル状の重なりや、複雑な宝飾チェーンのような西洋式ネックレスの形状は採用しない。
  - 翡翠と黒曜石の原石から巨大に粗く切り出された、**重なり合わない「単一の円状のネックレス（single, non-overlapping circular strand）」**。
  - 石器で叩き削ったばかりの粗い切削跡や天然の鉱脈がそのまま残り、太い無染色の麻紐で無造作に結ばれている。泥臭くも力強い意志の象徴。
...
(for divine elements): the same Washi + Iwa-enogu medium, but with ultra-refined disciplined brushwork, controlled minimal bleeding, dignified traditional palette (gofun white, muted gold leaf, ultramarine, vermilion), stillness and symmetry,
...
- **トーンの拡張:** タチバナとワカヒコのビジュアルには互いに「血の赤」を共通のアクセントとして入れ、ピアサポート的な共鳴を視覚的にも示唆する。
- **毒色の扱い:** スクナの周囲にはホルミシスを喚起させる酸緑・傷紫を積極的に使い、画面の一部だけが生き物のように毒に満ちているように見せる。
- **機能美の視覚化:** マヒトの装備には「加工硬化」を連想させる亀裂と溶接跡、傷が硬度を増す表現を追加。
- **同一媒体内コントラスト厳守:** 人間側は奔放な滲みと土色、神（敵・システムUI）側は制御された筆致と格調色を徹底する。
...
- **雰囲気:** 水彩の滲みは毒や薬品を想起させる酸緑や傷紫。背景は危険な実験室の汚れと毒液の飛沫。
  - **アニメーションステート:** 通常は歩行時に乳鉢を引きずる低速歩行、調合時に乳鉢を据え置きする静止モーション、必要時に「背負う」「杵で打撃する」の２つの瞬発アニメーションを使い分ける。各状態は周辺の煙や毒液エフェクトで視覚的に区別。
```text
Prompt:
...
```
### 3.3 うかみ（野生の斥候／行者）
- **コンセプト:** 本編では猿田彦的に渋みと威厳のある中年・壮年の姿が正史となる。若々しい25歳前後の美青年版はサイドエピソードやエピローグ向けのIFとして管理する。
...
- **キーアクション:** マヒトは鏡を打ち終えた後、ミコトから「赤いスカーフ」を受け取る。彼はそのスカーフで、まだ熱を帯びた鏡の表面をゆっくりと、しかし力を込めて磨き上げる。
- **ビジュアルエフェクト:** スカーフが鏡に触れると、スカーフに染み込んだ泥や血、そしてミコトが旅で蓄積した「情念の熱」が、赤い光の粒子となって鏡面に吸い込まれていく。磨き上げられた鏡面には、完璧な反射ではなく、まるで使い古された道具のように、温かい光のむらや、微細な「金継ぎ」のような模様が浮かび上がる。これは「破壊」の記憶ではなく、「修復」の歴史が刻まれた証である。

## 8. 旧構想（不採用要素のアーカイブ）

- **裏ボス「澱神・八岐の剣塚」のビジュアル（旧構想）:** 残骸と黄泉の泥だけで構成されるキメラ像。「八岐の産土」へとアップデートされたことに伴い、前半の白化殻（天の理への模倣）と後半の極彩色ノイズ（溜め込んだ情念の爆発）という二段構成へと昇華させたため旧案となる。
