...
- **Weapon_Category_Master:** 武器カテゴリ（剣、直刀、大剣、槍、御幣、双鎌、巨大やっとこなど）ごとに素材制約（王道武器 vs 日用品/祭具）、共有可能キャラクターリスト、フリクション基準を定義。**キャラクター1人につき5〜6種のカテゴリへの適性（持ち込み枠）**が設定される。煩雑な積載重量ペナルティは撤廃され、装備枠（インベントリ）は**「王道武器×2枠、日用品／祭具×1枠」等**の固定スロット制で管理される。これにより、各キャラクターのトラウマや個性を反映した多彩な神写し（技の継承）ネットワークを構築する。
