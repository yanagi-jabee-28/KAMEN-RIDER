...
- **代受苦発動条件とデータ遷移:**
  ...
  - **特別仕様（完全消滅の二者択一）:** 任意の選択により「武器の完全ロスト」を伴う極大の代受苦を行った場合のみ、`Item_Instance`がデータベース上から完全消去され、以下のどちらか1つを生成する。
...
- **Item_Master:** 武器、装束（従来の防具枠を完全統合）、形代（装飾品）、金継ぎ素材、修復触媒アイテムを定義。
  ...
  - `Remnant_Bone`: 極大の代受苦（ロスト）後の遺骨。元の武器IDと特性を保持し、「呼び継ぎ」によるキメラ作成に使用される。
