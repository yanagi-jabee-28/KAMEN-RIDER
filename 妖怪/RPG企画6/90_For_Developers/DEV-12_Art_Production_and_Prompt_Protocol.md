...
uid: [DEV-12]
project_code: RPG企画6
title: Art Production and Prompt Protocol
role: developer-art-production
status: active
...
# [DEV-12] Art Production and Prompt Protocol

このドキュメントは、Artist/Prompt担当向けの「画風ルール」と「プロンプト集」をまとめたファイルです。
目的は、誰が描いても同じ世界観に着地できるよう判断基準を固定することです。

## 制作導線（迷わないための順序）

- まず「1. ビジュアル・フィロソフィー」を固定し、素材断絶を避ける。
- 次に「2. スタイルアンカー」を全キャラ共通の土台として使う。
- その上で「3. キャラクタービジュアル定義」「4. 神・敵ビジュアル定義」へ進む。

## ART-40との責務境界

| 文書 | 主目的 | 書いてよい内容 | 書かない内容 |
|---|---|---|---|
| ART-40 | 非開発者向けの美術方針共有 | 世界観、色、質感、キャラ/環境の見た目方針 | 実装手順、命名規則、出力管理手順 |
| DEV-12 | 制作担当の再現可能な実務手順 | プロンプト本文、命名規則、出力・格納・レビュー手順 | 物語本文、公開向けの導入説明 |

境界に迷った場合は、先にART-40へ「意図」を書き、DEV-12へ「手順」を書く。

## 5分で再現する制作手順（最短版）

1. DEV-12の対象セクションからベースプロンプトを選ぶ。
2. 幕タイトル対応（白/滲/赫/金/還）に合わせて色と質感の語彙を調整する。
3. 禁止語（3D, CGI, 近代SF語）をNegative Promptへ入れて逸脱を防ぐ。
4. 出力ファイル名を `幕_対象_版数_日付` で保存し、同名でレビュー記録を残す。
5. 最後にART-40の該当方針と照合し、差分がある場合はART-40側の意図を優先して再生成する。
...
注: 付喪神化、金継ぎ、神託崩壊などの固有語彙は変更しない。

## 幕タイトルとビジュアルの関係
本作の五つのサブタイトルは、各幕の色・光・質感を決める実務基準です。
第1幕は「白堊の胎動」を軸に、強いコントラストと硬い線を使います。
第2幕は「灼鉄の滲転」として、ブラシストロークと水彩の滲みを強めます。
第3幕の「星火の逆説」は、赤いフレアと粒子飛散の基準です。
第4幕と終幕では、「砕界の金脈」と「永劫の放熱」に対応した金の再接合と土の温度感を反映します。
...
複雑な制約を減らし、世界観の表現力とAIの想像力を引き出すためのシンプルなベースプロンプトです。
...
### UIとシステムのアート方針
UIも「静止した神の理（神UI）」と「傷ついた人間の痕跡（人UI）」の対比で設計する。
金継ぎ補修、和紙テクスチャ、岩絵具の発色をシステムグラフィックへ直接反映する。

### 鍛造UI段階表現（Lv0〜Lv3）
- `Lv0`: 泥色の布と砥石のみ。火花はほぼ出さない。
- `Lv1`: 細い金継ぎ線と最小限の火花。修復成功時に細い金粉が走る。
- `Lv2`: 鍛造音・火花・煤の演出を強化。付喪神化時は武器に金継ぎ脈動エフェクトが同期する。
- `Lv3`: 神社背景の祈り粒子と鍛造火花を重ね、神話的錬成の到達点として見せる。
