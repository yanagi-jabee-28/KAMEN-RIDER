...
| `WEAPON_SABI` | 錆 | 武器耐久の軽度劣化。行動後に耐久消費が増えやすい | 戦闘/探索の摩耗 |
| `WEAPON_SHOKU` | 蝕 | 武器耐久の重度劣化。行動後に耐久消費が大きく増えやすい | 戦闘/探索の摩耗 |
| `WEAPON_KUCHI` | 朽 | 武器耐久の極危険劣化。現在耐久が残っていても、行動ごとの耐久消費率が極端に上がり、突発破損リスクが非常に高い | 戦闘/探索の摩耗 |
...
>
> 補足2: 武器耐久度は「現在どれだけ行動できるか」の現在値、劣化段階（錆/蝕/朽）は「器としての傷み具合」の状態値として別軸で管理する。
...
| 劣化進行 | 錆 / 蝕 / 朽 | 戦闘後や探索後にも武器の傷みが残る。段階は耐久度の高低と同義ではない | 野営での手入れ・鍛造を優先し、朽は即時対処する |
...
| `WEAPON_DEGRADE` | Lv.1 | 錆 | 武器劣化の初期段階 |
| `WEAPON_DEGRADE` | Lv.2 | 蝕 | 武器劣化の進行段階 |
| `WEAPON_DEGRADE` | Lv.3 | 朽 | すぐに破損へ至りやすい極危険段階 |
