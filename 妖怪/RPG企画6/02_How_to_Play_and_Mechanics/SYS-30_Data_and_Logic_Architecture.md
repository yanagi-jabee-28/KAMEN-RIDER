...
#### 2.4a 禁忌鍛造による武器進化の分岐（マヒト拠点Lv別）
マヒトの禁忌鍛造は段階的に解放され、段階ごとに主腕A（基盤武器）と主腕B/C（サブウェポン）の進化が並行します。各段階での耐久消耗・活魂摩耗リスク係数も増加します。

| 鍛造段階 | 拠点Lv要件 | 解禁フラグ | 主腕A進化先 | 主腕B/C進化先 | リスク係数 |
|---|---|---|---|---|---|
| **Lv0**（野営型） | 野営 | `MAHITO_JOINED_ACT2` | 煤まみれの大槌→刃折りの業槌 | 錆びたやっとこのみ（進化不可） | 1.0倍（通常） |
| **Lv1**（野外鍛造） | 拠点Lv1-2 | `MAHITO_FIELD_LV2_UNLOCKED` | (刃折りの業槌) | 灼熱の鉄鉗へ進化（星の砂Lv1接合） | 1.5倍（武器過熱加速） |
| **Lv2**（神器鍛造） | 拠点Lv3（神器） | 第4幕中盤カガセオ破断 | 天目の神火槌へ覚醒 | 炎神の牙鉗へ覚醒（星の砂Lv2完全接合） | 2.0倍（限界コスト） |
