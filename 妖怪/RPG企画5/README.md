---
uid: [ARC-INDEX]
project_code: RPG企画5
title: アシブネノミコト 〜天降る御子と、星屑の大地〜
status: draft
owner: Architecture Guardian
depends_on:
  - 00_Governance/ARC-00_Implementation_Charter.md
influences:
  - 01_Worldbuilding/WRD-01_Core_Vision_and_Theme.md
  - 02_Narrative/NAR-10_Narrative_and_Characters.md
  - 03_Systems/SYS-20_Game_Systems_and_Flow.md
  - 03_Systems/SYS-30_Data_and_Logic_Architecture.md
  - 04_Art/ART-40_Art_Direction_and_Assets.md
  - 05_References/REF-50_Reference_DQ_Master_Data.md
---

# RPG企画5 — アシブネノミコト 〜天降る御子と、星屑の大地〜

---

## このゲームについて（初めて読む方へ）

日本神話をベースにした、暗めのファンタジー RPG です。
壊れた武器を直しながら旅を続け、最後に「神の永遠」に立ち向かう物語です。

| 項目 | 内容 |
|---|---|
| タイトル | アシブネノミコト 〜天降る御子と、星屑の大地〜 |
| ジャンル | 和風神話ダークファンタジー RPG |
| 遊び心地 | 武器を直したり、仲間に受け継いだりしながら進む、泥臭い旅 |
| 見た目 | 一枚の和紙に岩絵具で描いたような絵。神は細くきれいな線、人は荒々しい滲みで表現 |

### この世界の対立

神々は「大切な人を失う悲しみをなくしたい」という気持ちから、世界を止めようとしています。
人間たちは「傷ついてもいい、それでも生き続けたい」という気持ちで、それに抵抗します。
神が悪いわけではなく、どちらも「悲しみをなくしたい」という思いからきている点がポイントです。

**舞台となる世界**

| 場所 | 説明 |
|---|---|
| **高天原** | 何も変わらない、完璧で冷たい神の場所 |
| **葦原中国** | 人や国津神が生き死にしながら暮らす、普通の世界 |
| **黄泉の国** | 神に捨てられた命が積もる場所。回復すら毒になる |
| **常世の国** | 神が目指す「完全に止まった美しい世界」。最後のダンジョン |
| **根の堅州国** | スサノオが流れ着いた、神の手が届かない場所 |

### 主人公と仲間

**ミコト（アシブネノミコト）:**
もともと天の命令で動いていた、感情のない「器」でした。星神カガセオとの戦いで記憶をすべて失い、葦舟に乗せられて海に流されます。漁村アマに流れ着いたとき、何も覚えていない状態からスタートします。地上の人たちと旅をする中で、少しずつ「ただの人間」になっていく話です。

**仲間（加入する順番）**

| 名前 | 加入 | どんな人か |
|---|---|---|
| **うかみ** | 第1幕 | 漁村アマの住人。ミコトに赤いスカーフを巻きつけた最初の人。第2幕で一度離れ、第4幕で再び戻ってくる |
| **スクナ** | 第1幕 | 薬師。皮肉っぽいが、劇薬で道を切り開く |
| **ウズ** | 第2幕前半 | 「狂騒」という技で、敵の完璧な守りを内側から破る |
| **タチバナ** | 第2幕中盤 | 忘却の海食洞で出会う少女。「誰かを生かすために死んでもいい」という執念を持つ |
| **マヒト** | 第2幕中盤 | 片目の鍛冶師。彼が加入すると、武器に魂を宿らせる「禁忌鍛造（Lv2）」が使えるようになる |
| **ワカヒコ** | 第3幕 | 天の監視役。ナキメとの戦いの後、一行に加わる |

**主な神々（主に敵側）**

| 名前 | 役割 |
|---|---|
| **イザナギ** | この物語の最終的な意志。世界を止めようとしている張本人 |
| **アマテラス** | 世界の管理者。悲しみをなくすために天岩戸に閉じこもり、世界の時間を止めた |
| **ツクヨミ** | 命を「永遠の眠り」に閉じ込めようとする。実はアマテラス封印後の世界を辛うじて保っている側面もある |
| **カガセオ（星神）** | 天に最初に逆らった者。砕けた破片が「星の砂（金継ぎの光）」の正体 |
| **ヒルコ** | 神話で最初に捨てられた命。第3幕でミコトの過去を暴く |

### バトルの仕組み

戦闘で使うリソースは3つだけです。新しいゲージは増えません。

| リソース | 意味 | 尽きたら |
|---|---|---|
| **活魂（Kakkon）** | 体力 | 戦闘不能。条件を満たすと「死狂い」に入る |
| **情念（Jonetsu）** | 技を使うための気力 | 「空殻」状態になり、技が使えなくなる |
| **武器耐久度** | 武器がどれだけ持つか | 武器が壊れる。金継ぎで直すまで使えない |

**死狂い（Shigurui）:**
活魂がゼロになっても、情念が残っていて、専用のパッシブを持っていれば、体が崩れながらも動き続けられます。この状態では神の攻撃を無視できますが、情念を使い切ると消えます。

**金継ぎと付喪神化:**
使い続けた武器は傷の歴史を蓄えます。金継ぎで直すたびに、武器に少しずつ意志が宿ります。十分な歴史を積んだ武器だけが「付喪神化（禁忌鍛造 Lv2）」し、最強の技「極大代受苦」が使えるようになります。強い武器を探すゲームではなく、使い続けた武器を繋ぎ直すゲームです。

**神と人の戦い方の違い:**

| 神（敵） | 人（プレイヤー） |
|---|---|
| 次の行動を完全に予告する。無駄がない | 神の計算を外れた、むちゃくちゃな熱量でぶつかる |
| プレイヤーの行動パターンを学習する | 死の淵に飛び込んで「死狂い」で予告を破壊する |
| 世界を止めようとする | 壊れながらも動き続ける |

### 物語の流れ（全5幕）

| 幕 | サブタイトル | 主な出来事 |
|---|---|---|
| **第1幕【胎】** | 白の輪郭 | 漁村アマに流れ着く。うかみ・スクナと旅を始め、天の軍勢に追われて村を離れる |
| **第2幕【融】** | 滲む境界 | 白堊の回廊で完全に負ける。タチバナ・マヒトが加わり、禁忌鍛造が使えるようになる。うかみが離れる |
| **第3幕【熾】** | 赫の瞬き | ワカヒコが加わる。ヒルコと出会い、ミコトがかつて何をしてきたかが明らかになる |
| **第4幕【結】** | 金の脈動 | 黄泉に下る。カガセオとの戦い。うかみが戻ってくる。天岩戸を強制的に開く |
| **終幕【還】** | 還り着く熱 | 常世〜高天原へ。神の「永遠の停滞」を打ち砕き、死ぬ「ただの人間」として終わる |

### どのファイルから読めばいい？

```
README.md（このファイル）
  └─ ① 世界観・テーマの根拠
        WRD-01_Core_Vision_and_Theme.md  ← 迷ったら必ずここに戻る
  └─ ② 物語・キャラクターの詳細
        NAR-10_Narrative_and_Characters.md
  └─ ③ ゲームシステムの設計意図
        SYS-20_Game_Systems_and_Flow.md
  └─ ④ 数値・データの正本
        SYS-30_Data_and_Logic_Architecture.md
  └─ ⑤ ビジュアル・演出
        ART-40_Art_Direction_and_Assets.md
  └─ ⑥（編集に参加する方のみ）運用ルール
        ARC-00_Implementation_Charter.md → ARC-01_UID_Registry.md
```

| よくある質問 | 答え |
|---|---|
| テーマを一言で言うと？ | 「壊れながらも生き続ける命」 vs 「悲しみをなくすために世界を止める神」 |
| 主人公は何者？ | 天が作った感情のない器。人と旅をするうちに初めて「人間」になる |
| なぜ武器が壊れる仕様なの？ | 「傷の歴史を積んだ武器こそが強い」というテーマを、ゲームの仕組みで表現している |
| 日本神話のどの部分を使っている？ | イザナギ・イザナミ、天岩戸、ヒルコ、スサノオ、カガセオなど。ただし独自解釈あり |
| ドラクエの要素がある？ | DQ のシステムをこのゲームの言葉に直して取り入れている。そのまま使うことはしない（詳細は [REF-50](05_References/REF-50_Reference_DQ_Master_Data.md)）|

---

## 編集者向け：運用ルール

このディレクトリは、RPG企画4の知見を引き継いで再構築したプロジェクトです。

> 運用宣言（2026-03-11）: 編集は `妖怪/RPG企画5` のみで行う。`妖怪/RPG企画4` は参照専用とする。

- [ARC-00]: 変更前に、どのファイルに影響するかを調べる。
- [ARC-01]: すべての正本ドキュメントに UID、depends_on、influences を付ける。
- [WRD-01]: SF語彙を禁止し、神話語彙へ統一する。

## 編集前の必読事項

> **このファイルだけ読んでも編集はできない。** 以下の2ファイルを先に読むこと。

### 1. 語彙ルール — SF語彙は混入禁止

ゲーム内テキスト（セリフ・アイテム説明・システムメッセージ）では、現代語・SF語彙を使わない。
変換表の全体は [ARC-00 §1.1](00_Governance/ARC-00_Implementation_Charter.md) に定義されている。

代表例：

| 禁止語 | 使うべき語 |
|---|---|
| AI / システム | 神託 / 神意 |
| ナノマシン | 祟りの粒 / 呪い |
| データ | 神話記録 / 神の理 |
| クラッシュ | 破綻 / 崩落 |

なお、仕様書・開発ドキュメント内での実務的な技術語（UI、バグ、Tick等）は許容されている（[ARC-00 §1.2](00_Governance/ARC-00_Implementation_Charter.md) 参照）。
ただし、物語本文や演出文などゲーム内テキストへ転用され得る記述では、SF語彙を可能な限り避けること。スクナの医学・薬学語彙は例外として許容する。

### 2. コアテーマ — 全仕様の判断基準

本作の世界観は **「永遠の結晶化（停滞）vs 傷を抱えた命の還流（変転）」** の一文で要約される。

- **天の神々（イザナギの理）**：悲哀を恐れ、世界を不変の琥珀に凍らせようとする。
- **地上の者たち**：壊れたものを金継ぎしながら、命を次代へ還流させようとする。
- **主人公ミコト**：天の使者として降された「空の器」が、地上の熱に触れ「ただの人間」へ脱皮する。

この対立軸に反する記述は書かない。詳細は [WRD-01 §2](01_Worldbuilding/WRD-01_Core_Vision_and_Theme.md) を確認してください。

### 2.5 時代考証の扱い — 許容と禁止

本作は神話ファンタジーです。厳密な史実再現は目的にしていません。時代考証の揺らぎは、神話語彙と世界観整合が保てる範囲で許容します。

- 許容: 常世由来・祭具由来・神意由来として説明できる異時代要素。
- 許容: 神術・魔術・呪法・怪異譚など、オカルト文脈で世界観と整合する要素。
- 非許容: 未来文明を直接示す要素（電子計算機器、近未来兵器、現代工業機械、ネットワーク前提の描写）。

迷った場合は [ARC-00 §1.3](00_Governance/ARC-00_Implementation_Charter.md) の判定基準に従い、判断理由を `WRD-99` に記録してください。

### 3. 変更手順 — UIDと影響範囲を必ず書く

1. 変更対象の UID を明示する。
2. `depends_on` と `influences` を確認し、影響範囲を整理する。
3. 本文を変更し、関係ファイルの内容もあわせて更新する。

手順の詳細と影響範囲分析テンプレートは [ARC-00 §2〜3](00_Governance/ARC-00_Implementation_Charter.md) を確認してください。

### 4. 正本の優先順

矛盾が生じた場合は以下の順で上位ファイルを優先する：

1. `03_Systems/SYS-30_Data_and_Logic_Architecture.md`
2. `03_Systems/SYS-20_Game_Systems_and_Flow.md`
3. `01_Worldbuilding/WRD-01_Core_Vision_and_Theme.md`

## ディレクトリ構造

- `00_Governance/`: 実装方針、UID台帳、運用ルール。
- `01_Worldbuilding/`: コア思想、世界観、アーカイブ。
- `02_Narrative/`: 物語設計、人物設計、幕構造。
- `03_Systems/`: 体験設計、数理設計。
- `04_Art/`: ビジュアル設計、演出。
- `05_References/`: 外部参照資料。

## `ALL-files` の使い方

`ALL-files` フォルダは、主要ドキュメントの**コピー**を置く場所です。
このディレクトリは**追跡除外**にし、リポジトリへコミットしないでください。
（`.gitignore` に `ALL-files/` を追加するのが手っ取り早いです。）

シンボリックリンクではなくファイルを複製するのは、
他者に渡すときや、VS Code・検索ツールで直接開きたいときに扱いやすいためです。

**自動同期が必要な場合**

ALL-files 配下の各ファイルを「個別シンボリックリンク」に差し替えると、
元ファイルを編集した内容をリンク先へすぐ反映できます。
以下はディレクトリごとではなく **ファイル単位で**リンクを張る例です。

```powershell
# 例: 00_Governance 内の Markdown を個別リンクに置換する
cd '...\妖怪\RPG企画5'

# 既存コピーを削除
Get-ChildItem -Path '..\ALL-files' -Filter *.md | Remove-Item

# 元ファイルをループしてリンク作成
Get-ChildItem .\00_Governance -Filter *.md | ForEach-Object {
    $target = $_.FullName
    $link   = Join-Path '..\ALL-files' $_.Name
    New-Item -ItemType SymbolicLink -Path $link -Value $target
}

# 他のサブフォルダでも同様。
# たとえば 01_Worldbuilding:
# Get-ChildItem .\01_Worldbuilding -Filter *.md | ForEach-Object { ... }
```

PowerShell の `New-Item -ItemType SymbolicLink` はファイル単位でも機能します。
ジャンクション (`-ItemType Junction`) はディレクトリ専用のため、個別ファイル
では上記のように `SymbolicLink` を使ってください。リンクを作成すると、
直接編集した同じファイルが ALL-files でも反映されます。

リンクは Git 管理下に含めても問題ありませんが、**リンク先が存在しないと
無効になる**ので、クローン先でも同じディレクトリ構造が必要です。


他のマシンへクローンした際には、ALL-files フォルダを作成して最新のコピーを取得します：
```powershell
cd '...\妖怪\RPG企画5'
mkdir '..\ALL-files' -Force
python refresh_allfiles.py
```
```
コピーを作成する手順（PowerShell）:

```powershell
# 例: ARC-00 をコピーする
cd '...\妖怪\RPG企画5'
Copy-Item -Path '.\00_Governance\ARC-00_Implementation_Charter.md' -Destination '..\ALL-files'
```

新しいドキュメントを追加したら、同じように `Copy-Item` を実行して
`ALL-files` へファイルをコピーしてください。複数ファイルをまとめて
コピーするワンライナーやスクリプトを `Scripts/` に置いておくと便利です。

現在は `妖怪/ALL-files` を通常フォルダとして使っています。ジャンクション運用に戻す場合は、`妖怪/ALL-files` を削除して `New-Item -Path .\ALL-files -ItemType Junction -Value .\RPG企画5` を実行してください。

なお、コピーされたファイルは元と内容が同じですが、以降の編集は元
ファイルで行い、必要に応じて同期を手動で行ってください。Gitには
どちらも含めることができますが、重複が煩雑になる場合は `ALL-files`
を `.gitignore` に追加する運用も検討してください。

## ドキュメント一覧

UID | パス | ステータス | 説明
---|---|---|---
ARC-00 | 00_Governance/ARC-00_Implementation_Charter.md | active | 実装ルールブック（運用ルール）
ARC-01 | 00_Governance/ARC-01_UID_Registry.md | active | UID 台帳（UID と命名規則）
WRD-01 | 01_Worldbuilding/WRD-01_Core_Vision_and_Theme.md | active | 世界観・コアビジョン
WRD-99 | 01_Worldbuilding/WRD-99_Archive_and_Changelog.md | active | 廃棄案・変更履歴とアーカイブ
NAR-10 | 02_Narrative/NAR-10_Narrative_and_Characters.md | active | 物語・登場人物設計
SYS-20 | 03_Systems/SYS-20_Game_Systems_and_Flow.md | active | ゲームシステムと体験フロー
SYS-30 | 03_Systems/SYS-30_Data_and_Logic_Architecture.md | active | データ・論理アーキテクチャ（数値定義）
ART-40 | 04_Art/ART-40_Art_Direction_and_Assets.md | active | アート指針・プロンプト集
REF-50 | 05_References/REF-50_Reference_DQ_Master_Data.md | reference-only | 外部参照資料（RPG4 ドラクエデータ）

## RPG企画4からの移行マップ

- `妖怪/RPG企画4/00_Core_Vision_and_Theme.md` -> `01_Worldbuilding/WRD-01_Core_Vision_and_Theme.md`
- `妖怪/RPG企画4/10_Narrative_and_Characters.md` -> `02_Narrative/NAR-10_Narrative_and_Characters.md`
- `妖怪/RPG企画4/20_Game_Systems_and_Flow.md` -> `03_Systems/SYS-20_Game_Systems_and_Flow.md`
- `妖怪/RPG企画4/30_Data_and_Logic_Architecture.md` -> `03_Systems/SYS-30_Data_and_Logic_Architecture.md`
- `妖怪/RPG企画4/40_Art_Direction_and_Assets.md` -> `04_Art/ART-40_Art_Direction_and_Assets.md`
- `妖怪/RPG企画4/50_Reference_DQ_Master_Data.md` -> `05_References/REF-50_Reference_DQ_Master_Data.md`
- `妖怪/RPG企画4/99_Archive_and_Changelog.md` -> `01_Worldbuilding/WRD-99_Archive_and_Changelog.md`

## 移行方式

- 方針: 再編集優先（重複削減・語彙統一・依存関係正規化）
- 優先順: `SYS-30` -> `SYS-20` -> `WRD-01` -> `NAR-10` -> `ART-40` -> `REF-50` -> `WRD-99`
- 凍結対象: `妖怪/RPG企画4`（参照専用）

> **アーカイブ注意:** 旧フォルダ内の7ファイルにはそれぞれYAMLメタデータ
> が追加され、`status: frozen` と `migrated_to` が記録されました。編集は
> 一切行わず、必要な箇所はRPG企画5側で参照または修正してください。
