...
このドキュメントは、RPG企画5の実装要件と変数定義をまとめた最上位ファイルです。物語的・情動的な表現は `../02_Narrative/NAR-10_Narrative_and_Characters.md` と `SYS-20_Game_Systems_and_Flow.md` に任せ、ここでは変数・フラグ・計算式を定義します。バランス数値を変更する場合は、このファイルを更新してください。

## 幕タイトルがコードに与える影響
本ファイルの設計には、各幕サブタイトルの位相（固・液・プラズマ・再結晶・相転移）を前提として組み込んでいます。たとえば第2幕では「滲む境界」を表現するため、環境圧フラグ `FreezeVacuum` や `BloodMud` の計算式を液相的挙動に寄せています。第3幕のピーク戦闘では共鳴イベントトリガー `Spark_Flare` でプラズマ相の爆発的発生を再現します。終幕の `HeatDissipation` 係数は「還り着く熱」の相転移特性を反映するため、数値調整時はサブタイトルの位相を参照してください。
...
// 追加: 装備スロットは原則「主腕1つ」の設計であるが、ミコトのみ
// 『副武器（Offhand Weapon）』スロットを持つ。これは一般的な副腕装備とは異なり、
// すべてのキャラクターで常時有効になるわけではない。
//
...
//
// この方式により、見た目上は両手装備を可能としつつ、他キャラクターへの
// 副武器概念の波及を抑制し、装備スロットの整合性を維持する。
