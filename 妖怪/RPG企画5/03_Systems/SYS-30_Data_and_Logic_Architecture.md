...
このドキュメントは、RPG企画5の実装要件と変数定義をまとめた最上位ファイルです。
物語的・情動的な記述は `../02_Narrative/NAR-10_Narrative_and_Characters.md` と `SYS-20_Game_Systems_and_Flow.md` が担当し、ここでは変数・フラグ・計算式を定義します。
バランス数値を変更する場合は、このファイルを更新してください。

## 実装読解ルール（Zero-Loss）

- 条件式、変数名、係数、閾値は原文をそのまま保持する。
- 平易化が必要な場合は、式の下に補助行を追加して説明する。
- 用語統一に迷った場合は `ARC-00` の保護語彙一覧を参照する。

### 補助語彙（説明用）

- SSOT: 唯一の正本となるマスターデータ
- 境界状態: 活魂と情念の組み合わせで遷移する戦闘状態
- 共鳴過熱: 新ゲージを増やさず、情念と武器耐久度を直接操作する増幅処理

## 幕タイトルがコードに与える影響
本ファイルの設計は、各幕サブタイトルの位相（固・液・プラズマ・再結晶・相転移）を前提に組み込む。
例として、第2幕は `FreezeVacuum` と `BloodMud` を液相寄りに調整し、第3幕は `Spark_Flare` で爆発位相を再現する。
終幕の `HeatDissipation` は相転移特性を反映するため、調整時は幕位相を参照する。
...
### 術式駆動リソース（三条の熱源）
本作に汎用的な「魔法値（MP）」はありません。以下の3リソースで術式と状態を管理します。
...
2. **情念（Jonetsu_Value）**:
  ...
  UI上はMPと同様に「残り値が減る」形で表示される。
    - **蓄積**: 基本ロジックは変わらず、被ダメージや味方ピンチで増加。
      ...
      ```
      消費時は `Jonetsu_new = Jonetsu_old - Cost` のように減算される。
      プレイ中は「必要コストを満たす残量があるか」で判断できる。
    - **回復手段（ゲーム中アクション）**:
      ```
...
// 追加: 装備スロットは原則「主腕1つ」。ただしミコトのみ
// 『副武器（Offhand Weapon）』スロットを持つ。
//
// データモデル上は `MainWeapon` と `OffhandWeapon` が存在するが、
// `OffhandWeapon` はミコト専用の特別枠であり、非ミコトは持たない。
// 副武器が装備されている場合でも通常攻撃時には主武器のみが有効であり、
// 特定術式や状態で `DualStanceActive` が真となるときのみ
// 「両手に武器を構える（Dual Wield）」状態として振る舞う。
// このとき、攻撃判定や耐久・消耗は両武器を参照し、手数や摩耗係数が増加する。
//
// この方式により、見た目上の両手運用を許容しつつ、
// 他キャラクターへの副武器概念の波及を抑えて整合性を維持する。
...
- `Stance_Multiplier`: 
  - **両手持ち（2H）**: **1.5**（重量武器の質量衝突で摩耗が最も激しい）。
    2H は「両手占有」の構えであり、ミコトの副武器スロット（Offhand Weapon）はこの構え中に無効化される。
  - **一刀流（1H）**: **1.0**（標準運用。攻撃力と摩耗の基準）。
    1H 武器同士を持つことでのみ、二刀流（Dual Wield）に移行できる。
  - **二刀流 / 仮想双腕**: **1.3**（`DualStanceActive == TRUE` 時のみ適用される特別スタンス。手数で押す代わりに摩耗ペースが上がる）

// `DualStanceActive` はミコト専用の一時フラグであり、
// 技や状態異常でのみ立ち上がる。副武器を装備していない場合は常に false となり、
// 装備していても通常攻撃時は主武器のみが参照される。
...
## 12. 新規状態異常とギミック（半実装）
以下は 2026-03-11 議論で追加提案された干渉名の半実装定義。
最終の倍率・付与率は未固定で、`SYS-20` の体験設計とプレイテストで確定する。
...
# Note
上記変数型は `bool` または `int` を基本とする。
半実装フェーズでは `Duration` / `Cooldown` / `Intensity` を暫定運用列として扱う。
...
- **Ukami_Heal_Absolute**: うかみの法力による回復。`Yomotsu_Eat_State` による回復反転の影響を内部的に無効化し、常にプラスの回復値として処理する。
  // `Field_State == BLOOD_MUDPIT` による回復反転をバイパスし、
  // 最終値へ直接加算して正常回復させる（絶対的救済）。
...
### Character_Equipment_Master（キャラクター別・装備スロット定数）
| Character_ID | 主腕（利用可能な共有カテゴリ） | 主腕（キャラ固有・専用武器） | 装束（メイン防具） | 形代（特殊アクセサリ枠） |
| --- | --- | --- | --- | --- |
| `MIKOTO` | 刀・槍 | 大幣（二刀流は特例仕様） | 星祝の防具（軽〜中装甲） | 泥濘の勾玉 |
| `UKAMI` | 刀・槍 | 錫杖（行者形態用） | 斥候の軽装 / 行者の法衣 | 風雷の獣牙 |
| `SUKUNA` | なし | 乳棒（劇薬調合用） | 薬草師の衣（耐化学の特注品） | 忘却の香炉 |
| `MAHITO` | 打撃 | 大槌・大やっとこ（鍛冶具の転用） | 鍛冶師の重装皮衣（耐熱重装）| 炉心の火打石 |
| `UZU` | 扇・刃・大幣 | 舞い用の鉄扇・隠し短刀 | 狂乱の舞衣（動きを阻害しない極軽装）| トランスの神楽鈴 |
| `TACHIBANA` | 槍・打撃・大幣 | 流木槍・流木の打ち櫂 | 濡れ羽の衣（常に湿気を帯びた装束）| 泥水の呪符 |
| `WAKAHIKO` | 遠距離・刃 | 天上弓・仕込み短刀 | 境界の隠秘服（迷彩効果を持つ装束）| 影縫いの鏑矢 |

### Weapon_Evolution_Master（武器の系譜記録）
...
| --- | --- | --- | --- | --- | --- |
| `MIKOTO` | 共鳴の核 | 光(星) / 土(泥) | 天の【光】を放ちつつ地の【泥】で受け止め無に帰す「器」 | 敵バフの解除・泥濘による遅延 | 神写しによるロール柔軟性が失われ、崩れた陣形の立て直しが困難になる |
| `UKAMI` | 野性の楔 | 土(地) / 風・雷 | 小細工を排し、正面から敵の理（神託）を力でへし折る「壁」 | 物理的威圧によるスタン・強制ヘイト・装甲(PTG)破壊 | 猛攻が脆い後衛に向かい、物理障壁を持つ敵に有効打を欠きジリ貧になる |
| `SUKUNA` | 劇薬師 | 毒(水/闇) / 炎・氷 | 神の理を単なる未解明現象と断じ、物理法則（薬学）で内部から腐食させる | 化学的崩壊（猛毒・即死・装甲無視の強酸・UI封じ） | 状態異常耐性を持てず、超高防御敵へのダメージリソースを完全に失う |
| `MAHITO` | 武具の守護者 | 炎(炉) / 土・氷 | 魔法ではなく純粋な質量と熱で神の機能（装備やバリア）をスクラップにする | 武器強制摩耗・バフの物理的粉砕 | 敵の無菌ガード等を解除できず、武器耐久過熱状態からのリカバリーが極めて困難になる |
| `UZU` | 拍子の破壊者 | 風(舞) / 光(狂) | 神の完璧な四拍子に裏拍で割り込み、未来の予定タイムラインを物理的にバグらせる | TICK攪乱（シャッフル）・トランス幻惑・混乱 | 完璧な計算通りに無慈悲な攻撃を受け続け、回避によるリソース節約が不可になる |
| `TACHIBANA` | 執着の打ち手 | 水(淵) / 闇(呪) | 海底へ沈められた息苦しさをそのまま敵のシステムへ流し込む広域デバッファー | 呪詛的重圧（全ステ低下・睡眠・ダメージ分散） | 対多数戦での全体無力化手段が乏しくなり、壁役突破時にパーティを護る命綱を失う |
| `WAKAHIKO` | 境界の狙撃手 | 光(天) / 闇(影) | 他と異なり、スナイパーとして冷徹に「ピンポイントに敵の計算を狂わせる」 | 戦術的阻害（行動順完全停止の影縫い・時限罠） | 絶対に行動させてはならない厄介な敵（即死・再生持ち）を遠距離から速攻で潰す手段を欠く |
...
### Status_Effect_Master（状態異常定義）
 | Effect_ID | 名称 | 説明 | 関連キャラクター(主使用/想定) |
 | --- | --- | --- | --- |
 | `POISON` | 毒 | 毎Tick活魂を減少させる継続ダメージ | スクナ、タチバナ |
 | `BAD_POISON` | 猛毒 | 毒の強化。復帰困難 | スクナ、タチバナ |
 | `PARALYSIS` | 麻痺 | 一定確率で行動不能 | スクナ |
 | `SLEEP` | 睡眠 | 次自身Tickまで行動不能。ダメージで解除 | タチバナ |
 | `CONFUSION` | 混乱 | ランダム行動または行動不発。UIの予測線が絡まり、計算式が壊れたバグ状態 | ウズ、(敵系統: 澱神、棄物など) |
 | `CHARM` | 服従 | 明確な殺意を持った味方への攻撃、または敵をかばう。敵の端正な計算式(UI)の駒として組み込まれた状態（敵専用） | (敵系統: 天津神、澱神など) |
 | `BLIND` | 幻惑 | 命中率低下。予測線は引けるが着弾点の座標計算に致命的なズレ（泥）が生じている状態 | ウズ、ワカヒコ |
 | `DESEASE` | 属性異常 | 属性耐性低下 | スクナ |
 | `STUN` | 怯み | 次の1Tickだけ行動を強制中断。予測線を構築する前に物理的な衝撃で計算ごと吹き飛ばされた状態 | うかみ |
 | `REST` | 休み | 1ターン確定行動不能（敵専用の自制等） | - |
 | `CURSE` | 呪い | 装備効果無効化 | タチバナ |
