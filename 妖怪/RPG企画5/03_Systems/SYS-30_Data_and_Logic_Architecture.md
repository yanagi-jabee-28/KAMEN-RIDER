...
| Character_ID | 中核役割 | 主属性 / 副属性 | 主腕構成 | 得意な干渉 | 欠けると起きる破綻（不在時のペナルティ） |
| --- | --- | --- | --- | --- | --- |
| `MIKOTO` | 共鳴の核 | 光(星) / 土(泥) | 刀 / 槍 / 大幣 / 二刀流は専用特例 | 敵バフの解除・泥濘による遅延 | 神写しによるロール柔軟性が失われ、崩れた陣形の立て直しが困難になる |
| `UKAMI` | 野性の楔 | 土(地) / 風・雷 | 斥候=刀/槍、行者=錫杖/槍/刀 | スタン・強制ヘイト・装甲(PTG)破壊 | 敵の猛攻が直接脆い後衛に向かい、物理障壁を持つ敵に有効打を欠きジリ貧になる |
| `SUKUNA` | 劇薬師 | 毒(水/闇) / 炎・氷 | 乳棒のみ、薬は行動として扱う | 猛毒・強酸（装甲無視）・即死 | 状態異常耐性を持てず、超高防御の敵（剥落の星屑等）へのダメージリソースを失う |
| `MAHITO` | 武具の守護者 | 炎(炉) / 土・氷 | 大槌 / 大やっとこ | 武器強制摩耗・バフ完全リセット | 敵の無菌ガード等を解除できず、武器過熱状態からのリカバリーが極めて困難になる |
| `UZU` | 拍子の破壊者 | 風(舞) / 光(狂) | 鉄扇 / 舞短刀 / 大幣 | 行動順(Tick)乱数化・トランス幻惑 | 敵の予告(完璧な計算)通りに無慈悲な攻撃を受け続け、回避によるリソース節約が不可になる |
| `TACHIBANA` | 執着の打ち手 | 水(淵) / 闇(呪) | 流木槍 / 流木の打ち櫂 / 大幣 | 全ステ低下・睡眠・ダメージ分散 | 対多数戦での全体無力化手段が乏しくなり、壁役突破時にパーティ全体を護る命綱を失う |
| `WAKAHIKO` | 境界の狙撃手 | 光(天) / 闇(影) | 天上弓 / 仕込み短刀 | 行動順完全停止(影縫い)・時限罠 | 絶対に行動させてはならない厄介な敵(即死持ち等)を、遠距離から速攻で潰す手段を欠く |
