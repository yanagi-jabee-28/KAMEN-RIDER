...
### Status_Effect_Master（状態異常定義）
 ...
 | `CONFUSION` | 混乱 | 行動がランダム化 | ウズ |
 | `CHARM` | 魅了 | 味方への攻撃を強制（敵専用） | (敵系統: 澱神など) |
 | `BLIND` | 暗闇 | 命中率低下 | ウズ、ワカヒコ |
 | `DESEASE` | 幻惑 | 属性耐性低下 | スクナ |
 | `STUN` | 怯み | 次の1Tickだけ行動を強制中断 | うかみ |
 | `REST` | 休み | 1ターン確定行動不能（敵専用の自制等） | - |
 | `CURSE` | 呪い | 装備効果無効化 | タチバナ |
 | `INSTANT_KILL` | 即死 | 条件で一撃死亡 | スクナ、ワカヒコ |
 | `SKILL_SEAL` | 封印 | 特技使用不可（凍結の真空含む） | (敵系統: 白化神など) |
 | `CRYSTALLIZE` | 琥珀化 | 完全停止状態（システム上の消滅扱いと同義） | (敵系統: 天津神など) |
