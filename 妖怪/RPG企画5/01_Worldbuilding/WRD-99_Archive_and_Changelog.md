...
このドキュメントは、プロジェクトで「不採用」と確定した案の履歴です。
ここにある内容は現行仕様として適用しません。判断の再確認と再発防止のために保持します。

## 参照ルール（誤読防止）

- このファイルは「履歴の保存」が目的であり、現行仕様の正本ではない。
- 実装判断は常に `SYS-30` → `SYS-20` → `WRD-01` の順で確認する。
- 本文中の旧語・旧仕様は、履歴証跡として保持される。現行適用の可否は各行の注記を優先する。

> 「何を作らないか」の履歴は、次の判断を速く正確にするための設計資産である。
...
この章は、直近の主要意思決定を時系列で記録した一覧です。
各行の「対象ファイル」を見れば、反映先を追跡できます。

- 2026-03-17: **【終盤導線A確定・天叢雲剣覚醒条件統一】** Gemini企画会議の決定を統合。①終盤順序を「本編カガセオ → クリア後スサノオ試練 → 真裏ボス澱神・八岐の産土」に確定。②「オロチ尾から剣が出る」神話を採用しつつ、単純取得を禁止し、露出した剣核をミコトの現行武器へ金継ぎ接合して天叢雲剣を覚醒させる方針へ統一。③ヒルコ・澱神・荒御魂の因果線（加害の履歴を鎮める贖罪導線）を維持したまま終盤導線を再配置。対象: `03_Systems/SYS-30_Data_and_Logic_Architecture.md`, `03_Systems/SYS-20_Game_Systems_and_Flow.md`, `02_Narrative/NAR-10_Narrative_and_Characters.md`, `01_Worldbuilding/WRD-01_Core_Vision_and_Theme.md`, `04_Art/ART-40_Art_Direction_and_Assets.md`, `README.md`。
- 2026-03-17: **【ミコトのバックボーン根本再構築・旅立ち動機のSF表現廃止】** Gemini企画会議の決定を統合。①ミコトの前歴を「神代の処刑者」として確立。神代においてカガセオとの決戦を直接行い、情念の逆流で全記録を焼失。天からパージされ葦舟で漂流の末に記憶なく漂着。タイトル「アシブネノミコト」とヒルコ神話を直接接続。②カガセオの「砕かれた」主体をミコトに特定（旧：曖昧な天の力→新：ミコトとの決戦）。星屑の荒野のカガセオ核を「人々の信仰で再結晶した未練の具現体」から「黄泉の底に沈んだ怨念の集合体（泥人形）」へ変更。③第1幕に「天の軍勢の二次襲撃」イベントを追加し、ミコトの離村動機を「防衛的離別」として確立（旧：SF的な「座標が示される」による受動的追従→新：自らの判断による能動的な旅立ち）。④「ヤマトへの動機」セクションから「空虚な闇の傷」「ヤマト深淵への座標が示される」を完全削除。二層構造（防衛的離別＋己の謎への問い）で置換。⑤第3幕ヒルコ遭遇を「真実を知る」から「加害の記憶フラッシュバック」へ昇格。⑥WRD-01のミコトの本性セクションに神代の処刑者歴→カガセオ決戦→全記録焼失→葦舟パージの因果連鎖を追記。対象: `02_Narrative/NAR-10_Narrative_and_Characters.md`（カガセオ定義・星屑の荒野定義・ヒルコとニニギセクション・神代の過去新セクション・第1幕二次襲撃追加・ヤマトへの動機書き換え）、`01_Worldbuilding/WRD-01_Core_Vision_and_Theme.md`（ミコトの本性セクション改訂）。
- 2026-03-16: **【天球三相変化の設計確定・ツクヨミ役割再定義】** Gemini企画会議の決定を統合。①世界の空の様相を「第一相（斜陽の猶予/第1幕〜第2幕前半）→第二相（天球の停止・永遠の極夜/第2幕終盤トリガーイベント）→第三相（絶対暗黒/ツクヨミ撃破直後）→第四相（昼夜の還流/終幕アマテラス解放後）」の四段階に確定。②ツクヨミを「過剰浄化の執行者」に加えて「天岩戸封印後はアマテラスの絶対零度の光から世界を辛うじて守護するフェイルセーフ（最後の傘）」として再定義。③静謐の塔を「ツクヨミの月引力によって中空固定された超質量楔」として物理的整合を明文化。④塔の落下原因を「ツクヨミ消滅＝月引力ストッパー喪失→物理落下＋天フェイルセーフ自動発動」として確定。⑤「ツクヨミを倒せば太陽が戻る」という民間伝承によるプレイヤーへのミスリードを設計として明示化。対象: `02_Narrative/NAR-10_Narrative_and_Characters.md`（天球三相変化セクション新規追加・天体モチーフ注記追加・§4ツクヨミ役割更新・第2幕静謐の塔記述詳化・第3幕偽終幕を絶対暗黒へ修正）。
...
- 2026-03-12: **【構え再定義】** 名称を「両手持ち／一刀流／二刀流」に統一し、耐久係数を 2H=1.5、1H=1.0、Dual=1.3 へ確定。旧語（剛・静・乱、GO/SEI/RAN）を正本から削除。
- 2026-03-12: **【ロジック整合修正】** ワカヒコ「返し矢の呪い」を恒常パッシブ化。生存の足掻きを条件成立時100%確定発動へ修正。剥落の星屑は熱散逸後に攻撃猶予Tickを設置してから逃走へ変更。黄泉戸喫は「手動摂取のみ在庫消費+呪い付与／うかみ自律摂取は内部法力で在庫非消費」を明文化。

- 2026-03-13: **【アーキテクチャ完全整合・冷却戦術追加】** 主要システム文書への一括改訂。
  1. 構えロジックをオフハンド装備依存から `DualStanceActive` 型仮想双腕へ転換し、主腕単一スロット設計を厳守。
  2. 旧語「魂のイデア/SoulIdea」を空前絶後に全廃し、正式語彙を「情念の核（Core_of_Regret）」へ統一。ノイズ耐性表・パラメータ名・説明文を更新。
  3. 情念の核生成条件と再抽出閾値を明文化し、付喪神化直後破壊では核が遺らない仕様を数理化。
  4. 黄泉の泥果実回復を減衰前基準で適用するよう順序を確定し、使用が侵食ゲージに影響しないことを明記。
  5. うかみの絶対回復 `Ukami_Heal_Absolute` が 黄泉呪い・血の泥沼両方を貫通する理論的優位性を追加説明。
  6. 澱神・棄物冷却を具現化する術式を追加（炉滓袋冷却、冷却の投擲など）、属性定義に ICE_COOLING を付与。
  7. 継承手甲を永久接続へ再定義し、装備/携行表現を撤廃。 Narrative と Systems 両面で表記を修正。
  8. 追加システム変数 `DualStanceActive` と `Mikoto_Gauntlet_Permanent` を導入。
  9. 画面文書と索引に関連語彙を反映。
...
### 1.10 武具再編会議の棄却履歴（2026-03-12）
- **旧案:** ミコト以外にも二刀流や副腕例外を広く開放し、各キャラクターへ複数主武器の同時運用を許す。
- **不採用理由:** 役割の輪郭が崩れ、誰が何で盤面を変えるのかが曖昧になる。主腕制限と常時携行物の再定義にも逆行する。
- **採用版:** 二刀流はミコト専用特例に限定し、他キャラクターは単独主武器と常時携行物の組み合わせで個性を立てる。

- **旧案:** うかみ行者期の主武器を数珠鎖へ寄せ、縛りと祈祷を武器カテゴリとして独立させる。
- **不採用理由:** 行者としての重量感が薄くなり、錫杖・石鉾・行者刀の三段構えよりも手触りが軽くなりすぎる。
- **採用版:** うかみは斥候刀と石鉾を基盤に、帰還後は錫杖中心へ移行する。数珠は祈りの象徴として常時携行に留める。

- **旧案:** スクナを乳棒と薬筒の二主武器運用にし、薬筒それ自体を打撃以外の主腕カテゴリとして扱う。
- **不採用理由:** 知恵者の身体性より道具の便利さが前に出てしまい、薬学による行動選択が武器枠へ吸われる。
- **採用版:** スクナの主武器は乳棒のみ。劇薬は散布・投与・抽出という行動へ分解して表現する。

- **旧案:** マヒトに鉄鎌、ウズに旗槍、タチバナに鉤鎖を持たせ、それぞれ補助武器で差別化する。
- **不採用理由:** 武器カテゴリが拡散しすぎ、鍛造・舞・海淵呪詛という各人の核よりも小道具の珍しさが前面化する。
- **採用版:** マヒトは大槌と大やっとこ、ウズは鉄扇・舞短刀・荒幣、タチバナは流木槍・打ち櫂・海淵の忌み幣へ整理する。
...
| 2026-03-07 | 企画会議（2026-03-07-13-31-14.md）の結果を全ドキュメントに反映。対立軸を「無菌世界」から「永遠の結晶化（停滞）vs 命の還流（変転）」へとシフト。アマテラスの「絶望」、ツクヨミのリデザイン（悲哀の神）、ミコトの役割（結晶世界を支える要石）を再定義し、神話的情念を強化。 | すべてのドキュメント | 
...
| 2026-03-15 | 【全仕様反映・戦術定義】各キャラクターへの主/副属性（神の静止vs人の冷却・摩耗概念に基づく）および、状態異常（理の崩し方）の役割マトリクスを追加。加入・離脱タイミングにおける戦術的不借ペナルティ（いるとできること/いないと困ること）を明文化。 | `02_Narrative/NAR-10_Narrative_and_Characters.md`, `03_Systems/SYS-20_Game_Systems_and_Flow.md`, `03_Systems/SYS-30_Data_and_Logic_Architecture.md`, `01_Worldbuilding/WRD-99_Archive_and_Changelog.md` |
| 2026-03-15 | 【状態異常の呼称差し戻し】一旦は「狂騒」「同化」「視界の泥濘」「凍結の真空」とした命令不可・妨害系状態異常の呼称を、従来の「混乱」「服従」「幻惑」「封印」という直感的な名称へ差し戻し。システム的・ナラティブ的意義（UIのバグ化、敵論理への組み込まれ）の定義自体は維持。 | `03_Systems/SYS-30_Data_and_Logic_Architecture.md`, `03_Systems/SYS-20_Game_Systems_and_Flow.md`, `02_Narrative/NAR-10_Narrative_and_Characters.md` |
