...
- **Ketsuwashin_Penalty**: キメラ化武器は耐久減少係数が **1.5〜3.0倍** に跳ね上がる（強力だが即消耗する）。
...
Can_Use_Ketsuwashin = (Current_Location_Level >= 2) AND Mahito_InParty
...
Extreme_Daijuku → Item_Instance を完全消去し以下を生成:
  ...
  - Route B: Core_of_Regret（情念の核・結和神化専用素材）
```
...
### Item_Master（主要フラグ）
 ...
 | `Core_of_Regret` | 極大代受苦後の情念の核。元の武器IDと特性を保持。結和神化専用素材（星の砂と混同させない） | 
...
### Item_State_Extension（武器インスタンス付加情報）
 ...
 | `Is_Ketsuwashin` | 結和神化フラグ。`TRUE`の場合再度の極限代受苦は情念の核を生成せず粉砕される | 
...
| `Kagaseo_Star_God` | カガセオ | 物理装甲ではなく高圧の情念でダメージ計算。砕かれた玉座への帰還引力と天の拒絶理が衝突し、特殊HP減算（暴走散逸）が発生する。 | 
 ...
 | `Boss_Yamata_no_Ubusuna` | 澱神・八岐の産土 | ステータスが `Global_Daijuku_Log_Data` と `Ketsuwashin_Craft_Count` で動的スケーリング。殻破壊後にUIジャック状態へ移行。 | 
...
## 5. バランス調整パラメータ（実数はプレイテストで決定）
 ...
 | `ResonanceRate` | 共鳴倍率 | 
 | `FrictionBase` | 基本摩擦係数 | 
 | `Ketsuwashin_Friction_Mult` | キメラ化時の摩擦倍率（1.5〜3.0） | 
